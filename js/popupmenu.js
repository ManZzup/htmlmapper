	...
	init:function($, $target, $popupmenu){
		...
		if (this.builtpopupmenuids.length==0){ //only bind click event to document once
			$(document).bind("click", function(e){
				el = e;
                                if (e.button==0 && !($(e.target).is(".jqpopupmenu select") || $(e.target).is(".jqpopupmenu div") || $(e.target).is(".jqpopupmenu option") )){ //hide all popup menus (and their sub ULs) when left mouse button is clicked
